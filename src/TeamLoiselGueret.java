 ...
 * @version $1.0$
 */
public class TeamLoiselGueret extends ControlSystemSS
{
	...
	private Vec2 getClosestPlayerToTarget(Vec2 target, Vec2[] players, boolean isMyTeam)
	{
		...
		else
		{
			...
			for (int i = 0; i < players.length; i++)
			{
				Vec2 copyPlayer = TeamMath.toward(target, players[i]);
	...
	{
		Vec2 vector = TeamMath.toward(target, destination);
		...
		if (ourside == -1)
			return TeamMath.isBetweenAngle(bottomAngle, topAngle
				  , bottomAngle2, topAngle2);
		else
			return TeamMath.isBetweenAngle(topAngle, bottomAngle
	...
	private boolean playerInFront(Vec2[] team)
	{
		for (int i=0; i<team.length; i++)
		{
			if(!isPassable(team[i]) && TeamMath.toward(team[i], ourgoal).r < 0.2)
			{
				return false;
			}
		}
	...
	private Vec2 defenderIsBetter()
	{
		...
		if (mynumber == 1)
		{
			if(TeamMath.toward(ball, ourgoal).r < 1.4)
			{
				result = betweenBallFromOurgoal();
				abstract_robot.setDisplayString("between ball");
				if (isClosestToBall())
		...
		{
			if(isClosestToBall() || TeamMath.toward(ball, ourgoal).r < 1.4){
				result = behindBallFromTheirgoal();
				abstract_robot.setDisplayString("behind ball");
	...
	private Vec2 strikerIsBetter()
	{
		...
		if (mynumber == 2)
		{
			if(TeamMath.toward(ball,ourgoal).r > 1.2)
			{
				result = behindBallFromTheirgoal();
				abstract_robot.setDisplayString("behind ball");
		...
		else if (mynumber == 4)
		{
			...
			else if(getClosestTeammateToBall().r < 0.8 && TeamMath.toward(ball,ourgoal).r > 1.6
